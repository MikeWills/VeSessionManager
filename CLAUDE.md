...
- **FCC's Grant Date does NOT advance when a licensee upgrades class — Last Action Date does.** A real General→Extra upgrade taken 2026-07-19 still reported Grant Date `2021-04-30` alongside Last Action Date `2026-07-21`. Any "did this exam produce a result?" check written against Grant Date is therefore correct for a first-time licensee and *permanently false* for an upgrade — that exact mistake left 20 real candidates pending indefinitely, the oldest 19 days. Confirming an upgrade needs two signals together: the operator class from **`AM.dat`** (present in every archive, joined by USI like HD/EN) matching `Candidate.NewLicenseClass`, **and** Last Action Date on/after the session. Neither alone is sufficient — class alone re-confirms someone who already held it walking in, date alone matches any unrelated administrative action. See `FccUlsWatcherService.ProcessLicensesAsync` and `docs/fcc-uls-watcher.md`'s "Confirming a class upgrade". Corollary: `Candidate.LicenseGrantDateUtc` holds Last Action Date for a confirmed upgrade, not the original grant, so `LicenseGrantPredatesSession()` returns false for upgrades granted after 2026-07-30.
