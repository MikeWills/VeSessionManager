...
- **Multi-team direction flagged, not yet built:** the user asked (2026-07-20) whether the app could someday serve multiple independent VE teams, each with their own Discord server/Square account/etc., while still sharing one FCC ULS download across all of them. The FCC watcher already supports this for free — its candidate queries span every `Vec`/session in one pass, no per-team concept needed. Zoom/Discord/Square/Email do not: each is a **singleton bound to one global appsettings/user-secrets credential set**. Multi-team support for those would mean moving credentials onto a per-`Vec` DB record and reworking each client from "one cached token for the app's lifetime" to "resolve credentials per-Vec per-call" — a real but bounded change, intentionally deferred as its own future design pass rather than folded into Phase 5.
