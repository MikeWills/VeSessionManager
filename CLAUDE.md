...
- **Multi-team foundation (2026-07-20):** the hierarchy is **VEC ⇒ Team ⇒ VE**, not the reverse — full design/rationale in `docs/multi-team.md`. `Team` (new entity) is the group of VEs operating a deployment, holding ExamTools/Zoom/Discord/Square/Email credentials; `Vec` is the FCC-recognized coordinating org and stays exactly what it always was — a **shared, global reference table** (one real-world "ARRL" row, not one per team), since a VEC dictates fees universally, not per-team. `Vec` is **not** owned by `Team` (an earlier draft of this design wrongly proposed `Vec.TeamId` — don't repeat that). `Session` gained `TeamId` as an independent second FK alongside its existing `VecId` — no relationship between `Vec` and `Team` themselves. **This slice covers ExamTools only** (the one hard-required integration) — `SessionIngestionService.RunAsync` now takes a `Team`, `ExamToolsClient` manages one independent `HttpClient`/cookie-jar/login-state per `TeamId` internally (still a single `AddSingleton`, no keyed DI), and `SessionIngestionJob` loops every `Team` for the ingestion step only. Zoom/Discord/Square/Email (+ the Web project's Square webhook route) remain global/single-account, deliberately deferred as a fast-follow that repeats this exact pattern — `docs/multi-team.md` is the template, including the one piece that needs genuinely new design (a per-team Square webhook route, since signature verification needs the right team's key *before* the payload can be parsed to find which team it belongs to). **Critical, easy to forget:** the seed migration leaves the seeded `Team` row's ExamTools credentials `NULL` (migrations must never contain real secrets) — ExamTools polling is broken until someone manually sets them via direct DB edit; see TODO.md.
