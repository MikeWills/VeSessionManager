...
- Phase 0 (foundation), Phase 1 (ExamTools session/candidate ingestion), Phase 2 (Zoom + Discord event scheduling), Phase 3 (Square payment links + webhook), Phase 4 (candidate notification emails + templates), Phase 5 (FCC ULS application/license watcher), Phase 6 (payment reminder & expiration job), Phase 7 (VE tracking), and Phase 8 (ARRL submission tracker) of `docs/spec.md` are implemented. Next up: Phase 9a (auth + scaffolding, first of four Phase 9 admin-backend sub-phases).
...
- ARRL submission tracker (2026-07-21): design notes in `docs/arrl-submission-tracker.md`. Unlike every prior phase, this one has **no background job at all** — submitting to ARRL is an out-of-band manual process, so `ArrlSubmissionService`/`ArrlSubmissionReportService` (`VeSessionManager.Core/ArrlSubmissions/`) exist purely as logic for Phase 9's (not yet built) admin UI to call, registered in DI already so they're ready. `MarkSubmittedAsync` is one-way (Not Submitted → Submitted only, no un-submit) and idempotent — calling it again on an already-`Submitted` session is a no-op that preserves the original `ArrlSubmittedDate`/`ArrlSubmittedByUserId` rather than letting a duplicate call (e.g. a UI double-click) reassign submission credit; a real state change also writes an `AuditLog` row. `GetPendingSubmissionCountAsync` counts non-cancelled sessions with at least one candidate in a terminal `ApplicationStatus` (`Granted`/`Failed`/`NotTested`) whose `ArrlSubmissionStatus` is still `NotSubmitted`. No schema change — confirmed via a throwaway `dotnet ef migrations add` that produced an empty migration before writing any code; `Session.ArrlSubmissionStatus`/`ArrlSubmittedDate`/`ArrlSubmittedByUserId` already existed from Phase 0.
