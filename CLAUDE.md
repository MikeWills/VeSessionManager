...
- **Any page/service resolving `SessionAccessScope` for a possibly-TeamLead user must load the user through `CurrentUserLoader.GetUserWithManagerAsync`, not the bare `userManager.GetUserAsync`.** `SessionAccessScope.GetEffectiveTeamId`'s TeamLead branch reads `user.ManagedByUser?.TeamId`, which needs that navigation eager-loaded — the bare `UserManager.GetUserAsync(ClaimsPrincipal)` never loads it, so a TeamLead would sign in successfully and silently see zero sessions. Not caught for a while since no page had ever exercised the TeamLead path before. See `docs/admin-auth.md`.
