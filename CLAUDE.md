...
- **All changes land on `main` via a PR — no direct pushes — by convention, not server enforcement.** Still discipline-only, but **the reason changed and the old one is no longer true**: this was private on a free plan, where branch protection 403s ("Upgrade to GitHub Pro or make this repository public"). **The repo is public now**, so branch protection is available on the free plan — `gh api .../branches/main/protection` returns `404 Branch not protected` (2026-08-13), i.e. simply unconfigured rather than unavailable. Enabling it is an open decision, not a blocked one. See `CONTRIBUTING.md`.
