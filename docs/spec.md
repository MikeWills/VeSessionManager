...
- **VE licence expiration tracking:** whether each `VolunteerExaminer` still holds a valid licence, and specifically whether one assigned to a session will be **expired on that session's date** — a VE must hold General or higher to serve. Not to be confused with the Renewal Monitor shipped in v0.1.4 (`docs/renewal-monitor.md`), which watches a hand-curated list of arbitrary call signs; this is the roster itself, populated automatically, and the session-relative check is the part the Renewal Monitor structurally cannot do. Most of the machinery already exists — see [issue #107](https://github.com/MikeWills/VeSessionManager/issues/107) for the open design questions.
