...
- **Reschedule/cancellation detection:** for already-known sessions, compare the polled data against the stored `Session` record each run:
  ...
  - If the session is removed/cancelled in the source data: set `Status = Cancelled`, `CancelledUtc = now`, hand off to Phase 2 for Zoom/Discord cleanup. **Open item:** confirm during this phase whether the HamStudy/ExamTools library exposes an explicit "cancelled" flag or whether a cancelled session simply disappears from the feed — the exact detection logic depends on which, so don't assume the signal shape until you're looking at real API responses.
