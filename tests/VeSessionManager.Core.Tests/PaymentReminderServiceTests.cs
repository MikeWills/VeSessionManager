...
public class PaymentReminderServiceTests
{
        ...
        DateTime? paymentReminderSentUtc = null)
    {
        ...
        var session = new Session
        {
            ...
            DurationMinutes = 60, Vec = vec, FeeConfiguration = feeConfiguration, Status = sessionStatus,
    ...
    public async Task Reminder_ExactlyFiveDaysSinceApplicationDateEntered_Fires()
    {
        ...
        await SeedCandidateWithPaymentAsync(dbContext, applicationDateEnteredUtc: Now.AddDays(-5));
        var sender = new FakeEmailSender();

        var result = await CreateService(dbContext, sender).RunAsync(CancellationToken.None);
    ...
    public async Task Reminder_BeforeFiveDays_DoesNotFire()
    {
        await using var dbContext = CreateContext();
        await SeedEmailSettingsAndTemplatesAsync(dbContext);
        await SeedCandidateWithPaymentAsync(dbContext, applicationDateEnteredUtc: Now.AddDays(-4));
        var sender = new FakeEmailSender();
    ...
    public async Task Reminder_AfterFiveDays_Fires()
    {
        await using var dbContext = CreateContext();
        await SeedEmailSettingsAndTemplatesAsync(dbContext);
        await SeedCandidateWithPaymentAsync(dbContext, applicationDateEnteredUtc: Now.AddDays(-6));
        var sender = new FakeEmailSender();
    ...
    public async Task Reminder_AlreadySent_DoesNotResend()
    {
        await using var dbContext = CreateContext();
        await SeedEmailSettingsAndTemplatesAsync(dbContext);
        await SeedCandidateWithPaymentAsync(dbContext, applicationDateEnteredUtc: Now.AddDays(-8), paymentReminderSentUtc: Now.AddDays(-3));
        var sender = new FakeEmailSender();
    ...
    public async Task Reminder_UnmatchedCandidate_NeverFires_NoApplicationDate()
    {
        await using var dbContext = CreateContext();
        await SeedEmailSettingsAndTemplatesAsync(dbContext);
        await SeedCandidateWithPaymentAsync(dbContext, status: CandidateApplicationStatus.Unmatched, applicationDateEnteredUtc: null);
        var sender = new FakeEmailSender();
    ...
    public async Task Reminder_GrantedCandidate_TerminalStatusSkipsIt()
    {
        await using var dbContext = CreateContext();
        await SeedEmailSettingsAndTemplatesAsync(dbContext);
        await SeedCandidateWithPaymentAsync(dbContext, status: CandidateApplicationStatus.Granted, applicationDateEnteredUtc: Now.AddDays(-8));
        var sender = new FakeEmailSender();
    ...
    public async Task Reminder_NotApplicablePayment_Skipped()
    {
        await using var dbContext = CreateContext();
        await SeedEmailSettingsAndTemplatesAsync(dbContext);
        await SeedCandidateWithPaymentAsync(dbContext, applicationDateEnteredUtc: Now.AddDays(-8), paymentStatus: PaymentStatus.NotApplicable);
        var sender = new FakeEmailSender();
    ...
    public async Task Reminder_CancelledSession_Skipped()
    {
        await using var dbContext = CreateContext();
        await SeedEmailSettingsAndTemplatesAsync(dbContext);
        await SeedCandidateWithPaymentAsync(dbContext, applicationDateEnteredUtc: Now.AddDays(-8), sessionStatus: SessionStatus.Cancelled);
        var sender = new FakeEmailSender();
    ...
    public async Task Expiration_ExactlyTenDays_Fires_SetsExpiredUnpaid_SendsToAdmin()
    {
        await using var dbContext = CreateContext();
        await SeedEmailSettingsAndTemplatesAsync(dbContext);
        await SeedCandidateWithPaymentAsync(dbContext, applicationDateEnteredUtc: Now.AddDays(-10));
        var sender = new FakeEmailSender();
    ...
    public async Task Expiration_BeforeTenDays_DoesNotFire()
    {
        await using var dbContext = CreateContext();
        await SeedEmailSettingsAndTemplatesAsync(dbContext);
        await SeedCandidateWithPaymentAsync(dbContext, applicationDateEnteredUtc: Now.AddDays(-9));
        var sender = new FakeEmailSender();
    ...
    public async Task Expiration_AfterTenDays_Fires()
    {
        await using var dbContext = CreateContext();
        await SeedEmailSettingsAndTemplatesAsync(dbContext);
        await SeedCandidateWithPaymentAsync(dbContext, applicationDateEnteredUtc: Now.AddDays(-11));
        var sender = new FakeEmailSender();
    ...
    public async Task Expiration_AlreadyExpired_DoesNotResend()
    {
        await using var dbContext = CreateContext();
        await SeedEmailSettingsAndTemplatesAsync(dbContext);
        await SeedCandidateWithPaymentAsync(dbContext, applicationDateEnteredUtc: Now.AddDays(-20), expiredUnpaid: true);
        var sender = new FakeEmailSender();
    ...
    public async Task Expiration_GrantedCandidate_TerminalStatusSkipsIt()
    {
        await using var dbContext = CreateContext();
        await SeedEmailSettingsAndTemplatesAsync(dbContext);
        await SeedCandidateWithPaymentAsync(dbContext, status: CandidateApplicationStatus.Granted, applicationDateEnteredUtc: Now.AddDays(-20));
        var sender = new FakeEmailSender();
    ...
    public async Task Expiration_NotApplicablePayment_Skipped()
    {
        await using var dbContext = CreateContext();
        await SeedEmailSettingsAndTemplatesAsync(dbContext);
        await SeedCandidateWithPaymentAsync(dbContext, applicationDateEnteredUtc: Now.AddDays(-20), paymentStatus: PaymentStatus.NotApplicable);
        var sender = new FakeEmailSender();
    ...
    public async Task Expiration_CancelledSession_Skipped()
    {
        await using var dbContext = CreateContext();
        await SeedEmailSettingsAndTemplatesAsync(dbContext);
        await SeedCandidateWithPaymentAsync(dbContext, applicationDateEnteredUtc: Now.AddDays(-20), sessionStatus: SessionStatus.Cancelled);
        var sender = new FakeEmailSender();
    ...
    public async Task ReminderAndExpiration_BothOverdueInOneRun_BothFire()
    {
        ...
        await using var dbContext = CreateContext();
        await SeedEmailSettingsAndTemplatesAsync(dbContext);
        await SeedCandidateWithPaymentAsync(dbContext, applicationDateEnteredUtc: Now.AddDays(-12));
        var sender = new FakeEmailSender();
    ...
    public async Task UnmatchedFlag_ExactlyWindowDays_Flags()
    {
        await using var dbContext = CreateContext();
        await SeedEmailSettingsAndTemplatesAsync(dbContext);
        var (candidate, _) = await SeedCandidateWithPaymentAsync(dbContext, status: CandidateApplicationStatus.Unmatched, dateRegisteredUtc: Now.AddDays(-5));
        var sender = new FakeEmailSender();
    ...
    public async Task UnmatchedFlag_BeforeWindow_DoesNotFlag()
    {
        await using var dbContext = CreateContext();
        await SeedEmailSettingsAndTemplatesAsync(dbContext);
        await SeedCandidateWithPaymentAsync(dbContext, status: CandidateApplicationStatus.Unmatched, dateRegisteredUtc: Now.AddDays(-4));
        var sender = new FakeEmailSender();
    ...
    public async Task UnmatchedFlag_AlreadyFlagged_DoesNotReflag()
    {
        await using var dbContext = CreateContext();
        await SeedEmailSettingsAndTemplatesAsync(dbContext);
        var (candidate, _) = await SeedCandidateWithPaymentAsync(dbContext, status: CandidateApplicationStatus.Unmatched, dateRegisteredUtc: Now.AddDays(-10));
    ...
    public async Task UnmatchedFlag_ReceivedCandidate_NotFlagged()
    {
        await using var dbContext = CreateContext();
        await SeedEmailSettingsAndTemplatesAsync(dbContext);
        await SeedCandidateWithPaymentAsync(dbContext, status: CandidateApplicationStatus.Received, dateRegisteredUtc: Now.AddDays(-10), applicationDateEnteredUtc: Now.AddDays(-10));
        var sender = new FakeEmailSender();
    ...
    public async Task UnmatchedFlag_CustomWindow_RespectsConfiguredValue()
    {
        await using var dbContext = CreateContext();
        await SeedEmailSettingsAndTemplatesAsync(dbContext);
        await SeedCandidateWithPaymentAsync(dbContext, status: CandidateApplicationStatus.Unmatched, dateRegisteredUtc: Now.AddDays(-3));
        var sender = new FakeEmailSender();

        var result = await CreateService(dbContext, sender, unmatchedReviewWindowDays: 3).RunAsync(CancellationToken.None);
    ...
    public async Task SmtpNotConfigured_SkipsRemindersAndExpirations_ButStillFlagsUnmatched()
    {
        await using var dbContext = CreateContext();
        await SeedEmailSettingsAndTemplatesAsync(dbContext);
        await SeedCandidateWithPaymentAsync(dbContext, applicationDateEnteredUtc: Now.AddDays(-20)); // due for both
        var (unmatchedCandidate, _) = await SeedCandidateWithPaymentAsync(dbContext, status: CandidateApplicationStatus.Unmatched, dateRegisteredUtc: Now.AddDays(-10));
    ...
    public async Task PurgedCandidate_ExcludedFromReminderAndFlagPasses()
    {
        await using var dbContext = CreateContext();
        await SeedEmailSettingsAndTemplatesAsync(dbContext);
        var (candidate, _) = await SeedCandidateWithPaymentAsync(dbContext, applicationDateEnteredUtc: Now.AddDays(-8));
