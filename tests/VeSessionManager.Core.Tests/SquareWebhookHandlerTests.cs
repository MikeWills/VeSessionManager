...
public class SquareWebhookHandlerTests
{
    ...
    private const string NotificationUrl = "https://vesessionmanager.example/webhooks/square";
    ...
    private static SquareWebhookHandler CreateHandler(AppDbContext dbContext) => new(
        dbContext,
        Options.Create(new SquareOptions { WebhookSignatureKey = SignatureKey, WebhookNotificationUrl = NotificationUrl }),
    ...
    private static string ComputeValidSignature(string body) =>
    ...
    private static async Task<Payment> SeedPaymentAsync(AppDbContext dbContext, string squareOrderId, PaymentStatus status = PaymentStatus.Unpaid)
    {
        ...
        var session = new Session
        {
            ...
            DurationMinutes = 60, Vec = vec, FeeConfiguration = feeConfiguration, CreatedUtc = Now
        };
    ...
    public async Task ValidSignature_CompletedStatus_MatchingOrderId_MarksPaymentPaid()
    {
        ...
        var payment = await SeedPaymentAsync(dbContext, "order-123");
        var body = PaymentUpdatedBody("order-123", "COMPLETED");
        var signature = ComputeValidSignature(body);

        var outcome = await CreateHandler(dbContext).ProcessAsync(body, signature, CancellationToken.None);
    ...
    public async Task InvalidSignature_ReturnsInvalidSignature_AndDoesNotTouchPayment()
    {
        ...
        var outcome = await CreateHandler(dbContext).ProcessAsync(body, "not-a-real-signature", CancellationToken.None);
    ...
    {
        await using var dbContext = CreateContext();
        await SeedPaymentAsync(dbContext, "order-123");
        var body = PaymentUpdatedBody("order-123", "COMPLETED");

        var outcome = await CreateHandler(dbContext).ProcessAsync(body, signatureHeader: null, CancellationToken.None);
    ...
    {
        await using var dbContext = CreateContext();
        var payment = await SeedPaymentAsync(dbContext, "order-123", PaymentStatus.Paid);
