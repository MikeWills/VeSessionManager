...
- [ ] **Move ExamTools config URL to the web/admin side** (issue [#18](https://github.com/MikeWills/VeSessionManager/issues/18)) — `ExamTools:BaseUrl` is currently one global, whole-deployment `appsettings.json` value shared by every team; this asks to make it configurable per-team so a "dev team" can run against `examtools.dev` for testing while real teams poll `alpha.exam.tools` in parallel, both from the same deployment. Two design options raised 2026-07-28 (not yet decided): (1) a free-text `Team.ExamToolsBaseUrl` override column, or (2) keep both hosts in appsettings and add a `Team.ExamToolsEnvironment` (Dev/Production) enum switch instead of a free-text URL. Relates to the `alpha.exam.tools`-is-really-production finding in the closed-session-ingestion fix above (`docs/examtools-api.md`).
